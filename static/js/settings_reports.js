(() => {
    ...
    function setUserOptions(selectId, users) {
        ...
        if (!users.length) {
            selectEl.innerHTML = '<option value="">کاربری یافت نشد</option>';
            return;
        }
    ...
    function accessBadge(allowed) {
        return allowed
            ? '<span class="status-badge active">مجاز</span>'
            : '<span class="status-badge inactive">غیرمجاز</span>';
    }
    ...
    function scopeText(restricted, values) {
        if (!restricted) return 'بدون محدودیت';
    ...
    function renderAccessReportRows() {
        ...
        if (!info.rows.length) {
            tbody.innerHTML = '<tr><td class="center-text muted" colspan="6" style="padding: 24px;">رکوردی برای نمایش وجود ندارد.</td></tr>';
            meta.textContent = `تعداد نتیجه: ${info.total}`;
    ...
    function renderUserScopeList(containerId, items) {
        ...
        if (!Array.isArray(items) || !items.length) {
            ul.innerHTML = '<li class="muted">بدون محدودیت</li>';
            return;
        }
    ...
    function renderUserAccess(payload) {
        ...
        summary.innerHTML = `
            <div class="settings-user-summary-grid">
                <div><strong>کاربر:</strong> ${esc(user.full_name || user.email || '-')}</div>
                <div><strong>نقش:</strong> ${esc(user.role || '-')}</div>
                <div><strong>وضعیت:</strong> ${user.is_active ? 'فعال' : 'غیرفعال'}</div>
                <div><strong>Project Scope:</strong> ${effective.projects_restricted ? 'محدود' : 'بدون محدودیت'}</div>
                <div><strong>Discipline Scope:</strong> ${effective.disciplines_restricted ? 'محدود' : 'بدون محدودیت'}</div>
            </div>
        `;
    ...
    function renderAuditRows() {
        ...
        if (!rows.length) {
            tbody.innerHTML = '<tr><td class="center-text muted" colspan="6" style="padding: 24px;">لاگی یافت نشد.</td></tr>';
            meta.textContent = `تعداد لاگ: ${total}`;
    ...
    async function loadMetadata(force = false) {
        ...
        setSelectOptions('reportProjectCode', state.projects, 'همه پروژه‌ها');
        setSelectOptions('reportDisciplineCode', state.disciplines, 'همه دیسیپلین‌ها');
    ...
    window.openSettingsAuditJsonModal = function openSettingsAuditJsonModal(index, field) {
        ...
        if (rowIndex < 0 || rowIndex >= state.auditLogs.items.length) {
            notify('error', 'رکورد لاگ یافت نشد.');
            return;
        }
        ...
        if (!norm(rawText)) {
            notify('error', 'برای این بخش، داده JSON ثبت نشده است.');
            return;
        }
    ...
    window.copySettingsJsonModal = async function copySettingsJsonModal() {
        ...
        try {
            ...
            notify('success', 'JSON کپی شد.');
        } catch (_) {
            notify('error', 'کپی JSON ناموفق بود.');
        }
    };
    ...
    window.loadSettingsAccessReport = async function loadSettingsAccessReport() {
        try {
            ...
            if (!filters.projectCode && !filters.disciplineCode) {
                notify('error', 'حداقل یکی از فیلترهای پروژه یا دیسیپلین را انتخاب کنید.');
                return;
            }
            ...
            notify('error', error.message || 'خطا در دریافت گزارش دسترسی');
        }
    };
    ...
    window.downloadSettingsAccessReportCsv = async function downloadSettingsAccessReportCsv() {
        try {
            ...
            const filters = selectedFilters();
            if (!filters.projectCode && !filters.disciplineCode) {
                notify('error', 'برای دانلود CSV حداقل یک فیلتر انتخاب کنید.');
                return;
            }
            ...
            notify('success', 'خروجی CSV دانلود شد.');
        } catch (error) {
            notify('error', error.message || 'دانلود CSV ناموفق بود');
        }
    };
    ...
    window.loadSettingsUserAccess = async function loadSettingsUserAccess(silentIfEmpty = false) {
        try {
            ...
            if (!userId) {
                const summary = document.getElementById('settingsUserAccessSummary');
                if (summary) {
                    summary.textContent = 'کاربر غیرادمین برای نمایش وجود ندارد.';
                }
                if (!silentIfEmpty) {
                    notify('error', 'کاربری برای نمایش انتخاب نشده است.');
                }
                return;
            }
            ...
            notify('error', error.message || 'خطا در دریافت دسترسی کاربر');
        }
    };
    ...
    window.loadSettingsAuditLogs = async function loadSettingsAuditLogs(options = {}) {
        try {
            ...
            notify('error', error.message || 'خطا در بارگذاری لاگ‌ها');
        }
    };
    ...
    window.initSettingsReports = async function initSettingsReports(force = false) {
        try {
            ...
            notify('error', error.message || 'بارگذاری تب گزارش‌ها ناموفق بود');
        }
    };
